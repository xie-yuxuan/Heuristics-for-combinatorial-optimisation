...
if __name__ == "__main__":
    for num_groups in range(4, 6):  # Loops from 3 to 7
        for t in range(10):  # Loops from t0 to t9
            graph_filename = f"SBM(1000, {num_groups}, t{t}).json"
            file_path = os.path.join('C:/Projects/Heuristics for combinatorial optimisation/Heuristics-for-combinatorial-optimisation/data/graphs', graph_filename)
    ...
    # file_path = r"C:\Projects\Heuristics for combinatorial optimisation\Heuristics-for-combinatorial-optimisation\data\graphs\SBM(1000, 2, t9).json"
            ...
            # specific_coloring = initial_node_colors[4]
            ...
            # g_optimised = np.array([graph_g.nodes[node]['color'] for node in graph_g.nodes()])

            # # print(f"g ground truth: {g}")
            # # print(f"g optimised: {g_optimised}")
            
            # nmi = normalized_mutual_info_score(g, g_optimised)
            # # print(g)
            # # print(g_optimised)
            # print(f"nmi: {nmi}")
            ...
            for i, initial_coloring in enumerate(initial_node_colors):
                ...
                results["cost_data"][f"initial_coloring_{i}"] = {
                    ...
                    "nmi_g" : normalized_mutual_info_score(g, g_optimised_g),
                    "nmi_r" : normalized_mutual_info_score(g, g_optimised_r)
                }
